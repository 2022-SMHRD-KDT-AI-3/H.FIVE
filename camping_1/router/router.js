...
router.get("/camp_info", function(request, response){
  ...
  let data = request.param('data');
  console.log("test"+data);
  
  let sql1 ="select * from camping where camp_id like '"+data+"';";
  //;  "+"select * from campImg where camp_id = "+data+"
  //"select * from camping where camp_id like '"+data+"';"+
  ...
  let sql2 = "select * from campImg where camp_id like '"+data+"';";
  conn.query(sql2,function(err, rows){
    console.log("test");
    data2 = rows;
    console.log(data1);
